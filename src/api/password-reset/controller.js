...
export const create = ({ bodymen: { body: { email, link } } }, res, next) =>
  User.findOne({ email })
    ...
    .then((reset) => {
      ...
      const content = `
        Hey, ${user.name}.<br><br>
        You requested a new password for your starcraftbuilds account.<br>
        ...
        &mdash; starcraftbuilds Team
      `
      return sendMail({ toEmail: email, subject: 'starcraftbuilds - Password Reset', content })
    })
